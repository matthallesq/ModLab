    ...
    <div className="w-[280px] h-full bg-white/80 backdrop-blur-md border-r border-gray-200 flex flex-col">
      ...
      <ScrollArea className="flex-1 px-4">
        <div className="space-y-1.5">
          ...
          {/* Project List - Only show when Projects is clicked */}
          {expandProjects && (
            <div className="pl-8 space-y-1 mt-1">
              {projects.map((project) => (
                <Button
                  key={project.id}
                  variant={"ghost"}
                  className={`w-full justify-start gap-3 h-9 rounded-xl text-sm font-medium ${activeItem === project.name ? "bg-blue-50 text-blue-600 hover:bg-blue-100" : "text-gray-700 hover:bg-gray-100"}`}
                  onClick={() =>
                    onProjectClick
